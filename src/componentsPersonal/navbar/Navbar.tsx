...
const Navbar = () => {
  ...
  const [scrolled, scrollY] = useScroll();
    ...
    const isActive = (obj: {isActive: boolean}) =>
        obj.isActive ? 'text-white  bg-accent p-3 rounded-none px-8' : 'text-white p-3 rounded-none px-8 hover:bg-accent';
  
  return (
<div className={(scrollY > 30 ? "scrolledY " : " ") + "navbar bg-transparent fixed z-50 sm:ps-16"}
  ref={scrolled}>
  <div className="navbar-start">
    <div className="dropdown">
        {/* MOBILE */}
      <div tabIndex={0} role="button" className="btn btn-ghost lg:hidden hamburger-menu ">
        <svg
      ...
      <ul
        ...
        className="menu menu-sm dropdown-content bg-base-100 rounded-box z-[1] mt-3 w-52 p-2 shadow">
         <NavLink onClick={()=>startPage()} to={routes.home}  className={isActive}>HOME</NavLink>
      
      <NavLink onClick={()=>startPage()} to={routes.platforms}  className={isActive}>PLATFORMS</NavLink>
    
    <NavLink onClick={()=>startPage()} to={routes.genres}  className={isActive}>GENRES</NavLink>
      </ul>
    </div>
    <a className="sm:text-5xl text-3xl font-bold lg:pt-7 flex text-accent">Ga<GiGamepad />er <span className="font-extrabold">X</span> .</a>
  </div>
  {/* PC */}
